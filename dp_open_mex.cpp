...
template <typename T>
void run_dp_open(const mxArray* ptsArg, mxClassID ptsClass, size_t nPts, size_t nVert, size_t* ind)
{
	...
	// run optimization
	std::vector<size_t> selVec = dp_open(ptsVec, nVert);
	...
	switch (mxGetClassID(ptsArg))
		{
			case mxSINGLE_CLASS:
			{
				run_dp_open<float>(ptsArg, mxSINGLE_CLASS, nPts, nVert, ind);
				break;
			}
			case mxDOUBLE_CLASS:
			{
				run_dp_open<double>(ptsArg, mxSINGLE_CLASS, nPts, nVert, ind);
				break;
			}
			default:
			{
